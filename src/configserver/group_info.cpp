 ...
 *
 * Version: $Id: group_info.cpp 2922 2014-09-05 08:25:26Z yunhen $
 *
 * Authors:
...
namespace tair {
  namespace config_server {
    ...
    int group_info::add_area_key_list(const char *group_file_name, const char *group_name, const char *area_key, uint32_t area_num)
    {
    ...
    int group_info::add_area_capacity_list(const char *group_file_name, const char *group_name, const char *area_key, uint32_t &area_num, const char *quota)
    {
      int ret = TAIR_RETURN_FAILED;
      ...
      if (strlen(area_key) == 5 && strncmp(area_key, "65536", 5) == 0)
      {  // if area_key == 65536, find available_area
    ...
    int group_info::update_area_capacity_list(const char *group_file_name, const char *group_name, uint32_t area_num, const char *quota)
    {
      int ret = TAIR_RETURN_FAILED;
      ...
      if (it != area_capacity_info.end()) { // if key exist, modify quota or delete qutao
        ...
        if (strlen(quota) == 1 && quota[0] == '0'){  // if quota == "0", remove this quota
          ...
          ret = util::file_util::change_conf(group_file_name, group_name, key, value, util::file_util::CHANGE_FILE_DELETE, ',');
          ...
          ret = util::file_util::change_conf(group_file_name, group_name, key, value, util::file_util::CHANGE_FILE_MODIFY, ',');
        }
      }
      else {  // if area does not exist, allcate a new area and add quota.
    ...
    void group_info::failover_server(node_info_set& upnode_list, bool& change)
    {
      ...
      for (std::set<uint64_t>::const_iterator it = pending_failover_servers.begin();
        ...
        if (failover_one_ds(server_id, upnode_list, change)) {
          log_warn("add server: %s into failovering_servers, failover begin", tbsys::CNetUtil::addrToString(server_id).c_str());
    ...
    bool group_info::failover_one_ds(uint64_t server_id, node_info_set& upnode_list, bool& change, bool check_alive)
    {
      ...
      do {
        ...
        if (!failovering_servers.empty()) {
          log_warn("server(%s) already in failover", tbsys::CNetUtil::addrToString(*failovering_servers.begin()).c_str());
          continue;
        }
    ...
    void group_info::recover_one_ds(uint64_t server_id, node_info_set& upnode_list, bool& change, set<uint64_t>& recovery_servers)
    {
      server_info sinfo;
      sinfo.server_id = server_id;
      node_info ninfo(&sinfo);
    ...
    int group_info::force_switch_bucket(set<uint32_t> bucket_nos, tbsys::CRWSimpleLock* p_grp_locker, tbsys::CRWSimpleLock* p_server_locker)
    {
      // hold all lock
      node_info_set upnode_list;
      p_grp_locker->wrlock();
        ...
        if (ret == TAIR_RETURN_SUCCESS)
        {
          for (set<uint32_t>::const_iterator iter = bucket_nos.begin(); iter != bucket_nos.end(); iter++)
          {
            ...
            log_warn("switch bucket %d : (%s, %s) => (%s, %s)", master_index,
                tbsys::CNetUtil::addrToString(master_server_id).c_str(), tbsys::CNetUtil::addrToString(slave_server_id).c_str(),
                tbsys::CNetUtil::addrToString(slave_server_id).c_str(), tbsys::CNetUtil::addrToString(master_server_id).c_str());
                              ...
                              tbsys::CRWSimpleLock* p_grp_locker, tbsys::CRWSimpleLock* p_server_locker)
    {
      ...
      if (ret == TAIR_RETURN_SUCCESS)
      {
        ...
        for (; iter != replace_ds_pairs.end(); iter++)
        {
          ...
          {
            log_error("server is not exist in available server. src_server: %s, dest_server: %s",
              tbsys::CNetUtil::addrToString(iter->first).c_str(), tbsys::CNetUtil::addrToString(iter->second).c_str());
            ret = TAIR_RETURN_FAILED;
            break;
          }
                       ...
                       tbsys::CRWSimpleLock* p_grp_locker, tbsys::CRWSimpleLock* p_server_locker)
    {
      ...
      if (ret == TAIR_RETURN_SUCCESS)
      {
        if ((*server_table_manager.migrate_block_count) != -1)
        {
          ...
          {
            log_error("%s failed when group is migrating as urgent offline server %s is still alive", option.ops_name,
        ...
        else if (failovering)
        {
          ...
          {
            log_error("%s failed when group is failovering as urgent offline server %s, but it's not in failover", option.ops_name,
                 tbsys::CNetUtil::addrToString(offline_id).c_str());
            ret = TAIR_RETURN_FAILED;
          }
        }
          ...
          if (got)
          {
            ...
            if (vec_replace_ids[j] == partner_id)
            {
              ...
              while (true)
              {
                ...
                if (k == j)
                {
                  log_error("%s failed as bucket %d is hold by (%s, %s), can't be replaced", option.ops_name, i,
    ...
    int group_info::force_table_ops_check(const ops_check_option& option)
    {
      ...
      do {
        ...
        {
          log_error("%s failed as server is migrating. migrate block count: %d", option.ops_name, *(server_table_manager.migrate_block_count));
          ret = TAIR_RETURN_FAILED;
          break;
        }
                             ...
                             tbsys::CRWSimpleLock * p_server_locker)
    {
      ...
      if (size <= (int) min_data_server_count) {
        ...
        log_debug("@@ need rebuild %d, failchange %d, recover change %d", need_rebuild, failover_change, recover_change);
        if (!need_rebuild) {
      ...
      *server_table_manager.recovery_block_count = -1;
      ...
      log_error("rebuild_table consume time : %lu us", 1000000 * (t_end.tv_sec - t_start.tv_sec) + t_end.tv_usec - t_start.tv_usec);
      // p_grp_locker->wrlock();
      if(ret == 0) {
    ...
    int group_info::packet_handler(easy_request_t *r) {
      ...
      if (rc == EASY_ERROR) {
        log_error("[%s] syn servertable to %s, timeout\n", group_name, easy_helper::easy_connection_to_str(r->ms->c).c_str());
      } else {
    ...
    void group_info::set_recovery_hashtable(size_t bucket_id, uint64_t server_id)
    {
      if(bucket_id > server_table_manager.get_server_bucket_count()
        ...
        log_error("bucket_id invalid or is_recoverying false. bucket_id(%lu), bucket_num(%d), recovery_block_count(%d)",
                  bucket_id, server_table_manager.get_server_bucket_count(), *server_table_manager.recovery_block_count);
        return;
      }
      ...
      {
        log_error("error happen, bucket_id: %lu, server: %s, m_server[0] is %s, m_server[1] is %s, d_server[0] is %s, d_server[1] is %s",
            bucket_id,
            tbsys::CNetUtil::addrToString(server_id).c_str(),
