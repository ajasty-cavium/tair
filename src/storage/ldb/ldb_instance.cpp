 ...
 *
 * Version: $Id: ldb_instance.cpp 2921 2014-09-05 03:52:52Z mobing $
 *
 * Authors:
    ...
    namespace ldb
    {
                           ...
                           bool version_care, int expire_time, bool mc_ops)
      {
        ...
        if (need_get)
        {
          ...
          if (TAIR_RETURN_SUCCESS == rc)
          {
            ...
            log_debug("@@ mtime care: %d,np:%d, %d <> %d, ldb_item.version:%d,key.data_meta.version:%d",
                        mtime_care, need_op,ldb_item.mdate(),key.data_meta.mdate,ldb_item.version(),key.data_meta.version);
            if (!need_op)
            {
        ...
        if (need_op && rc == TAIR_RETURN_SUCCESS)
        {
          ...
          log_debug("meta_version:%u ,prefix_size %u, total_size:%u, valuesize:%u edate:%u",ldb_item.meta().base_.meta_version_, key.get_prefix_size(), ldb_item.size(), value.get_size(), ldb_item.edate());
      ...
      int LdbInstance::update(int bucket_number, tair::common::data_entry& key, tair::common::data_entry& value, uint8_t ldb_update_type, bool version_care, int expire_time, bool mc_ops, data_entry *p_new_value)
      {
        if (db_ == NULL)
        {
          return TAIR_RETURN_SERVER_CAN_NOT_WORK;
        ...
        if (TAIR_RETURN_SUCCESS == rc)
        {
          ...
          log_debug("ldb::update %d, key len: %d, key prefix len:%d , value len: %u, update_type:%d version:%d expire:%d\n",
      ...
      int LdbInstance::add(int bucket_number, data_entry& key, data_entry& value, bool version_care, int expire_time)
      {
        ...
        if (TAIR_RETURN_DATA_NOT_EXIST == rc)
        {
          ...
          rc = do_put_and_set_meta(key, ldb_key, ldb_item, bucket_number, stat_data_size, stat_use_size, 1, expire_time, version_care, LDB_TYPE_ADD, &value);
          PROFILER_END();
        }
          ...
          bool is_incr, int expire, uint64_t &result, data_entry *p_new_value)
      {
        ...
        rc = do_put_and_set_meta(key, ldb_key, ldb_item, bucket_number, stat_data_size, stat_use_size, item_count, expire, true, type, NULL);
        if (rc == TAIR_RETURN_SUCCESS)
        {
          result = curr_val;
        }
      ...
      int LdbInstance::do_put_and_set_meta(data_entry& key, LdbKey& ldb_key, LdbItem &ldb_item, int bucket_number, int data_size, int use_size, int item_count, int expire, bool version_care, uint8_t ldb_update_type, data_entry *value)
      {
        ...
        log_debug("ldb::put, key len: %d, key prefix len:%d , value len: %u, version:%d expire:%d, mdate:%d, cdate:%d\n",
                   ...
                   ldb_item.meta().base_.version_, ldb_item.meta().base_.edate_
           , ldb_item.meta().base_.mdate_, ldb_item.meta().base_.cdate_);
        rc = do_put(ldb_key, ldb_item,
            SHOULD_PUT_FILL_CACHE(key.data_meta.flag),
            is_synced(key),
            is_meta_flag_unit(key.data_meta.flag),
        ...
        else if (still_have_)
        {
          ...
          while (batch_size < migrate_batch_size && batch_count < migrate_batch_count && scan_it_->Valid())
          {
            ...
            if (LdbKey::decode_bucket_number(scan_it_->key().data() + LDB_EXPIRED_TIME_SIZE) == scan_bucket_)
            {
              ...
              {
                log_error("kv size invalid: k: %d, v: %d [%zu %zu] [%x %x %s]", key_size, value_size, scan_it_->key().size(), scan_it_->value().size(), *(ldb_key.key()), *(ldb_key.key()+1), ldb_key.key() + 2);
                ::sleep(2);
              }
      ...
      int LdbInstance::do_get(LdbKey& ldb_key, std::string& value, bool from_cache, bool fill_cache, bool update_stat, bool mc_ops)
      {
          ...
          bool synced, bool from_other_unit, tair::common::entry_tailer* tailer)
      {
        ...
        if (UNLIKELY(!status.ok()))
        {
          ...
          stat_->update_physical_stat(LdbKey::decode_area_with_key(ldb_key.data()), 1, ldb_key.size()); // remove() actually add one key use_size
      ...
      void LdbInstance::sanitize_option()
      {
        ...
        options_.filter_policy = TBSYS_CONFIG.getInt(TAIRLDB_SECTION, LDB_USE_BLOOMFILTER, 0) > 0 ?
          new LdbBloomFilterPolicy(TBSYS_CONFIG.getInt(TAIRLDB_SECTION, LDB_BLOOMFILTER_BITS_PER_KEY, 10), stat_) : NULL;
        ...
        std::string time_range_for_specify_compact = TBSYS_CONFIG.getString(TAIRLDB_SECTION, LDB_SPECIFY_COMPACT_TIME_RANGE, "0-0");
        ...
        options_.kSpecifyCompactMaxThreshold = TBSYS_CONFIG.getInt(TAIRLDB_SECTION, LDB_SPECIFY_COMPACT_MAX_THRESHOLD, 10000);
        ...
        options_.kSpecifyCompactScoreThreshold = TBSYS_CONFIG.getInt(TAIRLDB_SECTION, LDB_SPECIFY_COMPACT_SCORE_THRESHOLD, 1);
    ...
    int LdbInstance::get_real_edate(int expire_time, int now, bool mc_ops)
      {
        ...
        if (expire_time > 0)
        {
          ...
          {
            edate = static_cast<uint32_t>(expire_time) >= (uint32_t) now - 30*24*60*60 ? expire_time : now + expire_time;
          }
        }
        else
        {
