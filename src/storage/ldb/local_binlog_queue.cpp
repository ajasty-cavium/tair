    ...
    namespace ldb {
      ...
      void InstBinlogQueue::delete_log_file(uint64_t filenumber) {
        if (filenumber > last_delete_log_file_) {
          log_warn("instance %d, filenum %" PRI64_PREFIX "u last delete log file %" PRI64_PREFIX "u",
      ...
      tair::common::RecordPosition InstBinlogQueue::get_min_position() {
        ...
        if (position.valid() == true) {
          ...
          log_info("instance %d: logger reading file number %" PRI64_PREFIX "u", index_, filenumber);
          ...
          uint64_t filenumber = logger_->get_reading_file_number();
          log_info("NEVER HAPPEN instance %d: logger reading file number %" PRI64_PREFIX "u", index_, filenumber);
        }
        ...
        if (block_logger_ != NULL) {
          ...
          if (position.valid() == true) {
            ...
            log_info("instance %d: block logger reading file number %" PRI64_PREFIX "u", index_, filenumber);
            if (min_position.valid() == false || position < min_position) {
              min_position = position;
