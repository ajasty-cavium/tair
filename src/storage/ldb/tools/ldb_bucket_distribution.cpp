 ...
 *
 * Version: $Id: ldb_bucket_distribution.cpp 1311 2013-01-23 05:24:56Z nayan $
 *
 * Authors:
...
int output_bucket_distribution(const leveldb::VersionSet& versions)
{
  ...
  for (int l = 0; l < leveldb::config::kNumLevels; ++l)
  {
    printf("level %d: ", l);
    int count = 0;
    for (std::set<int>::const_iterator iter = bucket_distribution[l].begin(); iter != bucket_distribution[l].end(); ++iter)
    {
  ...
  if (g_verbose)
  {
    for (int indx = 0; indx < MAX_BUCKET_NUMBER; ++indx)
    {
      ...
      if (!single_bucket_files.empty())
      {
        ...
        int count = 0;
        for (std::set<uint64_t>::const_iterator iter = single_bucket_files.begin(); iter != single_bucket_files.end(); ++iter)
        {
