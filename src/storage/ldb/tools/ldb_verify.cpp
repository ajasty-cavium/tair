 ...
 *
 * Version: $Id: ldb_verify.cpp 1327 2013-07-30 08:08:27Z ganyu $
 *
 * Authors:
...
static int              data_write_local(Configure *c, const char* keydata, int keysize, const char* valdata, int valsize, bool only_download_key);
...
static void             parse_cluster_string(std::vector<std::string>& infos, const char* line, const char* separator, const char* err_msg);
...
static std::pair<int, DestBucket*> do_remote_batch_put(Task* task, data_entry* key, data_entry* value, DestBucket* dbt = NULL);
...
static bool             verify_entry(bool remote, int area, const data_entry*, const data_entry*, const data_entry*, bool log_key);
static void             summary();
...
//      log_error("%u %s", mdate, sdate.c_str());
    ...
    if (c->prefix_key != "") {
      if (compare_cstr(c->prefix_key.c_str(), c->prefix_key.size(),
            key->get_prefix(), key->get_prefix_size()) == false) continue;
    }
...
bool
verify_entry(bool remote, int area, const data_entry *key, const data_entry *local_value, const data_entry *remote_value, bool log_key)
{
  bool ret = true;
