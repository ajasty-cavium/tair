 ...
 *
 * Version: $Id: ldb_bucket_check.cpp
 *
 * Authors:
 *   mobing <mobing.fql@taobao.com>
 *
 */
...
void CheckStat::do_dump(std::map<int32_t, Stat*>& stats, int32_t key, const char* desc)
{
  Stat* stat = NULL;
  fprintf(stderr, "%s\n", desc);
  fprintf(stderr, "%5s%12s%12s%12s%20s%15s%20s\n", "key", "totalCount", "totalSize", "sucCount", "invalidCount", "delOkCount", "delFailCount");
  ...
  if (key >= 0)
  {
    ...
    {
      stat = it->second;
      fprintf(stderr, "%5d%12ld%12ld%12ld%20ld%15ld%20ld\n", it->first, stat->count_, stat->size_, stat->suc_count_, stat->invalid_count_,
  ...
  else                        // dump all stats
  {
    ...
    fprintf(stderr, "%5s%12ld%12ld%12ld%20ld%15ld%20ld\n", "total", total_stat.count_, total_stat.size_, total_stat.suc_count_, total_stat.invalid_count_,
  ...
  else
  {
    for (size_t i = 0; !g_stop && i < buckets.size(); ++i)
    {
      ...
      for (db_it->Seek(leveldb::Slice(scan_key, sizeof(scan_key))); !g_stop && db_it->Valid(); db_it->Next())
      {
        ...
        stat.update(bucket, area, ldb_key.key_size() + ldb_item.value_size(), err_bucket == bucket, ret == TAIR_RETURN_SUCCESS);
      }
...
int main(int argc, char* argv[])
{
  ...
  if ((is_repair && (log_file_name == NULL || local_cluster_addr == NULL)) ||
      ...
      log_file_name == NULL || buckets == NULL || (bucket_num_str == NULL && local_cluster_addr == NULL))))
  {
    print_help(argv[0]);
    return 1;
  }
