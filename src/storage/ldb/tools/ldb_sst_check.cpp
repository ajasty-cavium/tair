 ...
 *
 *  Tool: check sst file by custom compare, default check all sst files
 *
 * Version: $Id: ldb_sst_check.cpp 1783 2013-08-20 05:24:56Z baichen $
 *
 * Authors:
 *   baichen <minghang.zmh@taobao.com>
 *
 */
...
bool check_table(leveldb::Env* env, const std::string& fname, leveldb::Comparator* comparator)
{
  ...
  for (iter->SeekToFirst(); iter->Valid(); iter->Next())
  {
    ...
    else
    {
      ...
      {
        printf("invalid key!!!\ncurr,type(%s)[%s]\nlast,type(%s)[%s] \n", type, EscapeString(key.user_key).c_str(), last_type, EscapeString(last_key).c_str());
        valid = false;
        break;
      }
