 ...
 *
 * tair ldb gc log reader
 *
 * Version: $Id: ldb_gclog_reader.cpp
 *
 * Authors:
...
void printCont(GcLogRecord glr)
{
  ...
  char buf[20];
  sprintf(buf, "%04d-%02d-%02d %02d:%02d:%02d", local->tm_year + 1900, local->tm_mon + 1, local->tm_mday, local->tm_hour, local->tm_min, local->tm_sec);
  char rslt[200];

  const char *ops = glr.oper_type_ == 1 ? "add" : "rem";
  const char *type = glr.node_type_ == 1 ? "bucket" : "##area";
  sprintf(rslt, "oper(%s), type(%s), key(%04d), sequece(%016" PRI64_PREFIX "u), filenum(%010" PRI64_PREFIX "u), time(%s)",
