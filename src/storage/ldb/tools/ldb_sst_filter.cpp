...
{
  fprintf(stderr, "%s: pick sstable by limited levels, output filenumber rename file and manifest of picked sstable.\n"
    ...
    for (size_t i = 0; i < metas.size(); ++i)
    {
      ...
      if (l == limit && !buckets.empty()) {
        ...
        for (int32_t b = smallest_bucket; b <= largest_bucket; ++b)
        {
          ...
          {
            std::cout << "#$ target bucket(" << b << ") found in level(" << l << "), file_number is " << f->number << std::endl;
            should_add = true;
            hit_cnt++;
            break;
          }
