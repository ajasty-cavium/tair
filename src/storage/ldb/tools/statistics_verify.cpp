 ...
 *
 * Tool: do checkup and rectify for the integrity of data of tair cluster(s) backed by ldb.
 *
 * Version: $Id: ldb_verify.cpp 1327 2015-04-13 10:08:27Z yunhen $
 *
 * Authors:
...
struct Configure
{
  ...
  {
    os<< "verify_type: "<< (LOCAL_MASTER_TO_REMOTE_MASTER_BUCKET == c.verify_type? "verify_local_master_bucket_to_remote_master_bucket" :
...
public:
  ...
  {
    return ((approximate_mget_return_packet_size + key->get_size() + 40 + ldb_item->value_size() + 40) > 400 * 1024) || (keys.size() + 1)> 200;
  }
...
private:
  ...
  int32_t do_success_get(tair_client_impl* client, int32_t area, const data_entry& key, data_entry*& local_value, int32_t server_select = 0)
  {
...
public:
  ...
  void add_key_and_check(leveldb::Iterator* itr)
  {
    ...
    {
      remote_key_out_mtime_range_statistics.add_stat(area, local_bucket_number, 1, ldb_key.key_size() + ldb_item->value_size());
      delete ldb_item;
      delete ldb_item_data;
      return;
    }

    data_entry* key = new data_entry(ldb_key.key() + LDB_KEY_AREA_SIZE, ldb_key.key_size() - LDB_KEY_AREA_SIZE, true);
    key->area = area;
    key->data_meta.flag = TAIR_CLIENT_DATA_MTIME_CARE | TAIR_CLIENT_PUT_SKIP_CACHE_FLAG;
  ...
  void finish()
  {
    map<int32_t, map<int32_t, AreaServeridKeys> >::iterator outer_itr = area_serverid_keys.begin();
...
parse_conf(Configure *c)
{
  ...
  if(success)
  {
    if (LOCAL_MASTER_TO_REMOTE_MASTER_BUCKET == c->verify_type)
    {
      ...
      success &= c->remote_client->startup(c->remote_master.c_str(), c->remote_slave.c_str(), c->remote_group.c_str());
    ...
    else
    {
      ...
      {
        log_error("verify is local, but local cluster's data copy count %d is not >=2", c->remote_client->get_copy_count());
        return false;
      }
    }
  }
