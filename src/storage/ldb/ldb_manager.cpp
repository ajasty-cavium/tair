 ...
 *
 * Version: $Id: ldb_manager.cpp 2921 2014-09-05 03:52:52Z mobing $
 *
 * Authors:
  ...
  namespace storage
  {
    namespace ldb
    {
      ...
      int LdbManager::touch(int bucket_number, data_entry &key, int expire_time)
      {
        ...
        {
          rc = db_instance->update(bucket_number, key, dummy/*not used...just pass by*/, LDB_TYPE_TOUCH, true, expire_time);
        }
        return rc;
      }
      ...
      int LdbManager::append(int bucket_number, data_entry &key, data_entry &value, bool version_care, data_entry *new_value)
      {
      ...
      int LdbManager::prepend(int bucket_number, data_entry &key, data_entry &value, bool version_care, data_entry *new_value)
      {
      ...
      int LdbManager::op_cmd(ServerCmdType cmd, vector<std::string>& params, std::vector<std::string>& infos)
      {
        ...
        if (TAIR_RETURN_SUCCESS == ret)
        {
          ...
          case TAIR_SERVER_CMD_SET_CONFIG:
          {
            ...
            else if (params[0] == LDB_BUCKET_INDEX_CAN_UPDATE)
            {
              ...
              const char* strategy = TBSYS_CONFIG.getString(TAIRLDB_SECTION,
                                    LDB_BUCKET_INDEX_TO_INSTANCE_STRATEGY, BI_STRATEGY_HASH);
          ...
          case TAIR_SERVER_CMD_GET_CONFIG:
          {
            ...
            os << LDB_BUCKET_INDEX_CAN_UPDATE << " = " << bucket_indexer_->get_index_update_flag(); //using different naming format to distinguishing with leveldb embeded configurations
      ...
      void LdbManager::ReviseStat::run(tbsys::CThread* thread, void* arg)
      {
        ...
        for (set<int32_t>::const_iterator it = instance_to_revise_.begin(); it != instance_to_revise_.end(); ++it)
        {
          ...
          int ret = instance->get_stat_manager()->start_revise(_stop, instance, is_revise_user_stat_, sleep_interval_, sleep_time_);
          if (TAIR_RETURN_SUCCESS == ret)
          {
