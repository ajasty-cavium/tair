 ...
 *
 *
 * Version: $Id: mdb_instance.cpp 1391 2013-03-12 12:13:05Z dutor $
 *
 * Authors:
...
namespace tair {
	...
	bool mdb_instance::initialize(mdb_area_stat *stat, int32_t bucket_count, bool use_share_mem /*=true*/)
	{
		...
		int meta_len = mem_pool::MEM_POOL_METADATA_LEN + mem_cache::MEMCACHE_META_LEN;        //cachehashmap will alloc space by itself
	...
	int mdb_instance::put(int bucket_num, data_entry & key, unsigned int hv, data_entry & value,
	...
	int mdb_instance::raw_put(const char* key, int32_t key_len, unsigned int hv,
			const char* value, int32_t value_len, int flag, uint32_t expired, int32_t prefix_size, bool is_mc)
	{
		...
		{        /* is evict */
			...
			if (LIKELY(is_item_expired_by_area_clear(it) == false))
				stat_->update(ITEM_AREA(it), -1, -(it->key_len + it->data_len),
											-cache->slab_size_of_item(it->item_id), evict_count);
			else
		...
		if (expired > 0) {
			if (is_mc) {
				it->exptime =
					...
					30*24*60*60 ? expired : crrnt_time + expired;
			} else {
	...
	int mdb_instance::raw_get(const char* key, int32_t key_len, unsigned int hv, std::string& value, bool update)
	{
	...
	int mdb_instance::raw_remove(const char* key, int32_t key_len, unsigned int hv)
	{
	...
	bool mdb_instance::raw_remove_if_exists(const char* key, int32_t key_len, unsigned int hv)
	{
		PROFILER_BEGIN("hashmap find");
		mdb_item *it = hashmap->find(key, key_len, hv);
		PROFILER_END();
	...
	bool mdb_instance::raw_remove_if_expired(const char* key, int32_t key_len, mdb_item*& item, unsigned int hv)
	{
		PROFILER_BEGIN("hashmap find");
		mdb_item *it = hashmap->find(key, key_len, hv);
		PROFILER_END();
	...
	int mdb_instance::add_count(int bucket_num, data_entry &key, unsigned int hv, int count, int init_value,
	...
	int mdb_instance::mc_set(int bucket_num, data_entry &key, unsigned int hv,
	...
	int mdb_instance::add(int bucket_num, data_entry &key, unsigned int hv,
			data_entry &value, bool version_care, int expire)
	{
		lock_guard guard(mdb_lock);
	...
	int mdb_instance::replace(int bucket_num, data_entry &key, unsigned int hv,
			data_entry &value, bool version_care, int expire)
	{
		lock_guard guard(mdb_lock);
	...
	int mdb_instance::append(int bucket_num, data_entry &key, unsigned int hv,
			data_entry &value, bool version_care, data_entry *new_value)
	{
		lock_guard guard(mdb_lock);
		return do_pending(bucket_num, key, hv, value, version_care, false, new_value);
	}

	int mdb_instance::prepend(int bucket_num, data_entry &key, unsigned int hv,
			data_entry &value, bool version_care, data_entry *new_value)
	{
		lock_guard guard(mdb_lock);
	...
	int mdb_instance::incr_decr(int bucket, data_entry &key, unsigned int hv, uint64_t delta, uint64_t init,
...
//    return area_stat[area].get_quota() <= area_stat[area].get_data_size();
  ...
  void mdb_instance::run_mem_merge(volatile bool &stopped)
  {
...
#if 1
	bool mdb_instance::get_next_items(md_info & info,
																	 vector<item_data_info *> &list, uint32_t bucket_count)
	{
		bool ret = true;
	...
	int mdb_instance::do_put(int bucket_num, data_entry & key, unsigned int hv, data_entry & data,
													bool version_care, int expire, bool is_mc)
	{
    ...
    if (mdb_param::enable_put_remove_expired) {
      it = hashmap->find_and_remove_expired(key.get_data(), key.get_size(), hv);
    } else {
      it = hashmap->find(key.get_data(), key.get_size(), hv);
		...
		if(it != 0) {                //exists
			...
			if (expired) {
				...
				TBSYS_LOG(DEBUG, "it->version(%hu) != version(%hu)", it->version,
									key.get_version());
				return TAIR_RETURN_VERSION_ERROR;
			} else {
				if (version_care) {
		...
		if (expire > 0) {
			if (!is_mc) {
				it->exptime =
					static_cast<uint32_t> (expire) >=
					crrnt_time - 30*24*60*60 ? expire : crrnt_time + expire;
			} else {
				it->exptime =
					static_cast<uint32_t> (expire) >
					30*24*60*60 ? expire : crrnt_time + expire;
			}
		} else if (expire < 0) {
				it->exptime = old_expire < 0 ? 0 : old_expire;
		} else {
		...
		TBSYS_LOG(DEBUG, "it->flags:%lx", it->flags);
	...
	int mdb_instance::do_get(int bucket_num, data_entry & key, unsigned int hv, data_entry & data, bool with_stat)
	{
  ...
  int mdb_instance::do_remove(int bucket_num, data_entry & key, unsigned int hv, bool version_care)
  {
	...
	int mdb_instance::do_add_count(int bucket_num, data_entry &key, unsigned int hv, int count, int init_value,
	...
	bool mdb_instance::do_lookup(int bucket_num, data_entry &key, unsigned int hv)
	{
		if (key.area != KEY_AREA(key.get_data())) {
		...
		if (is_item_expired(it, static_cast<uint32_t> (time(NULL)), bucket_num)) {
			__remove(it, hv);
			return false;
		}
		return true;
	...
	int mdb_instance::do_set(int bucket_num, data_entry &key, unsigned int hv, data_entry &value, bool version_care, int expire)
	{
		//TODO: reuse do_put for now
		return do_put(bucket_num, key, hv, value, version_care, expire, true);
	}

	int mdb_instance::do_add(int bucket_num, data_entry &key, unsigned int hv, data_entry &value, bool version_care, int expire)
	{
		//TODO: reuse do_put for now
		mdb_item *it = NULL;
		remove_if_expired(bucket_num, key, it, hv);
		if (it != NULL && version_care/*not mig/dup*/) return TAIR_RETURN_KEY_EXISTS;
		return do_put(bucket_num, key, hv, value, version_care, expire, true);
	}

	int mdb_instance::do_replace(int bucket_num, data_entry &key, unsigned int hv, data_entry &value, bool version_care, int expire)
	{
		//TODO: reuse do_put for now
		mdb_item *it = NULL;
		remove_if_expired(bucket_num, key, it, hv);
		if (it == NULL && version_care/*not mig/dup*/) return TAIR_RETURN_DATA_NOT_EXIST;
		return do_put(bucket_num, key, hv, value, version_care, expire, true);
	}

	int mdb_instance::do_pending(int bucket_num, data_entry &key, unsigned int hv,
			data_entry &value, bool version_care, bool pre, data_entry *new_value)
	{
		...
		if (version_care && key.get_version() != item->version && key.get_version() != 0) {
			return TAIR_RETURN_VERSION_ERROR;
		}

		int total_size = key.get_size() + value.get_size() + sizeof(mdb_item);
		total_size += item->key_len + item->data_len;
		if (total_size > TAIR_MAX_DATA_SIZE) return TAIR_RETURN_ITEMSIZE_ERROR;

		int type = key.area;
		mdb_item *new_item = cache->alloc_item(total_size, type);
		if (type == ALLOC_EXPIRED || type == ALLOC_EVICT_SELF || type == ALLOC_EVICT_ANY) {
			area_stat[ITEM_AREA(new_item)].decr_data_size(new_item->key_len + new_item->data_len);
			area_stat[ITEM_AREA(new_item)].decr_space_usage(cache->slab_size_of_item(new_item->item_id));
			...
			if (LIKELY(is_item_expired_by_area_clear(new_item) == false))
				stat_->update(ITEM_AREA(new_item), -1, -(new_item->key_len + new_item->data_len),
											-cache->slab_size_of_item(new_item->item_id), evict_count);
			else
				stat_->update(ITEM_AREA(item), mdb::STAT_EVICT_COUNT, evict_count);
		...
		if (pre) {
			...
			memcpy(ITEM_DATA(new_item) + sizeof(int32_t) + value.get_size(),
					ITEM_DATA(item) + sizeof(int32_t), item->data_len - sizeof(int32_t)); // origin
		} else {
			memcpy(ITEM_DATA(new_item), ITEM_DATA(item), item->data_len);
			memcpy(ITEM_DATA(new_item) + item->data_len, value.get_data(), value.get_size());
		}
		...
		stat_->update(ITEM_AREA(new_item), 1, new_item->data_len + new_item->key_len,
	...
	mdb_item* mdb_instance::alloc_counter_item(data_entry &key)
	{
		...
		int total_size = sizeof(int32_t) // flags
			+ max_ascii_counter_len
			+ sizeof(mdb_item) + key.get_size();
		int type = key.area;
		mdb_item *item = cache->alloc_item(total_size, type);
		if(type == ALLOC_EXPIRED || type == ALLOC_EVICT_SELF || type == ALLOC_EVICT_ANY) {
			area_stat[ITEM_AREA(item)].decr_data_size(item->key_len + item->data_len);
			...
			if (LIKELY(is_item_expired_by_area_clear(item) == false))
				stat_->update(ITEM_AREA(item), -1, -(item->key_len + item->data_len),
											-cache->slab_size_of_item(item->item_id), evict_count);
			else
				stat_->update(ITEM_AREA(item), mdb::STAT_EVICT_COUNT, evict_count);
			hashmap->remove(item);
		}
	...
	int mdb_instance::do_incr_decr(int bucket, data_entry &key, unsigned int hv, uint64_t delta, uint64_t init,
			bool is_incr, int expire, uint64_t &result, data_entry *new_value)
	{
		...
		if (item == NULL) {
			...
			if ((item->flags & TAIR_ITEM_FLAG_ADDCOUNT) == 0) {
				...
				curr_val = tair::util::string_util::strntoul(ITEM_DATA(item) + sizeof(int32_t),
						item->data_len - sizeof(int32_t));
				new_item = alloc_counter_item(key);
				new_item->exptime = item->exptime;
	...
	int mdb_instance::touch(int bucket_num, data_entry &key, unsigned int hv, int expire)
	{
		lock_guard guard(mdb_lock);
		int ret = TAIR_RETURN_DATA_NOT_EXIST;
		mdb_item *it = NULL;
		...
		if (!(expired = remove_if_expired(bucket_num, key, it, hv)) && it != NULL)
		{
	...
	void mdb_instance::close_buckets(const vector<int> &buckets, uint32_t bucket_count)
	{
  ...
  bool mdb_instance::remove_version_care(data_entry & key, unsigned int hv)
  {
	 ...
	 */
	bool mdb_instance::remove_if_expired(int bucket_num, data_entry & key, mdb_item * &item, unsigned int hv)
	{
		mdb_item *it = hashmap->find(key.get_data(), key.get_size(), hv);
		bool ret = false;
		if(it != 0) {                //found
	...
	{
		TBSYS_LOG(WARN, "%s: start removing expired items ...", instance_name);
		...
		log_warn("%s: end removing expired items, %ld items %ld bytes released", instance_name, items_removed, space_released);
		log_warn("%.9lf seconds consumed, duration per bucket:  max %.9lf, min %.9lf, avg %.9lf",
				sum_duration, max_duration, min_duration, sum_duration / hashmap->get_bucket_size());
  ...
  bool mdb_instance::is_mem_merge_time()
  {
    ...
    return (mdb_param::mem_merge_time_low != mdb_param::mem_merge_time_high &&
        hour_range(mdb_param::mem_merge_time_low, mdb_param::mem_merge_time_high));
  }
	...
	void mdb_instance::run_chkexprd_deleted()
	{
		...
		else
		{
			...
			if (last_expd_time != 0 && time(NULL) - last_expd_time < (60 * 60 * 6))
			{
	...
	void mdb_instance::keep_area_quota(bool &stopped, const map<int, uint64_t> &exceeded_area_map)
	{
		...
		while (!stopped && itr != exceeded_area_map.end()) {
			...
			log_warn("keeping quota of area %d, %lu bytes released, %.9fs consumed",
