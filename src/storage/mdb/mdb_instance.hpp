 ...
 *
 * Version: $Id: mdb_instance.hpp 1391 2013-03-12 12:13:05Z dutor $
 *
 * Authors:
...
//#define AREA(x) ((x) > TAIR_MAX_AREA_COUNT ? (TAIR_MAX_AREA_COUNT) : (x) )
  ...
  public:
    mdb_instance(int instance_id): instance_id_(instance_id), this_mem_pool(0), cache(0), hashmap(0),
                   last_traversal_time(0), last_balance_time(0), last_expd_time(0), last_check_quota_time(0),
  ...
  public:
    int raw_put(const char* key, int32_t key_len, unsigned int hv,
        const char* value, int32_t value_len, int flag, uint32_t expired, int32_t prefix_len, bool is_mc = false);
  ...
  private:
    ...
    int do_pending(int bucket_num, data_entry &key, unsigned int hv, data_entry &value, bool version_care, bool pre, data_entry *new_value);
