 ...
 *
 * Version: $Id: heartbeat_thread.hpp 2922 2014-09-05 08:25:26Z yunhen $
 *
 * Authors:
...
namespace tair {
   ...
   public:
      server_table_updater(tair_manager *tair_mgr, uint64_t *server_list, int server_list_count, uint32_t server_version, int32_t data_need_move, vector<uint64_t> current_state_table, uint32_t copy_count, uint32_t bucket_count, bool calc_migrate = true)
      {
   ...
   public:
      recovery_bucket_updater(tair_manager *tair_mgr, std::vector<uint64_t> recovery_ds, uint32_t server_version, uint32_t transition_version)
      {
         this->tair_mgr = tair_mgr;
      ...
      void run(tbsys::CThread *thread, void *arg)
      {
         ...
         if (tair_mgr != NULL) {
           if (recovery_ds.size() > 0) {
              log_warn("start to init work recovery_bucket_updater, server_version is %d, transition_version is %d", this->server_version, this->transition_version);
              tair_mgr->update_recovery_bucket(recovery_ds, transition_version);
           } else {
              log_warn("server_version or transition_version changed in failover status, but recovery ds is empty, stop current recovery process");
              tair_mgr->reset_recovery_task();
           }
         }
