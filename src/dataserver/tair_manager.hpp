 ...
 *
 * Version: $Id: tair_manager.hpp 2935 2014-09-09 09:35:24Z yunhen $
 *
 * Authors:
...
namespace tair {
   ...
   public:
      ...
      int prefix_incdec(request_prefix_incdec *request, int version, int low_bound, int upper_bound, bool bound_care, response_prefix_incdec *resp);

      int put(int area, data_entry &key, data_entry &value, int expire_time,base_packet *request=NULL,int version=0, bool with_stat = true);
      ...
      void update_server_table(uint64_t *server_table, int server_table_size, uint32_t server_table_version, int32_t data_need_remove, vector<uint64_t> &current_state_table, uint32_t copy_count, uint32_t bucket_count, bool calc_migrate = true);
      void direct_update_table(uint64_t *server_table, uint32_t server_list_count, uint32_t copy_count, uint32_t bucket_count, bool is_failover, bool version_changed = false, uint32_t server_table_version = 0);
