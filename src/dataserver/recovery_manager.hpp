...
namespace tair {
   ...
   public:
      recovery_manager(base_duplicator *this_duplicator, tair_manager *tair_mgr, tair::storage::storage_manager *storage_mgr);
      ~recovery_manager();
      void set_log(update_log *log);
      void set_recovery_server_list(bucket_server_map recovery_server_list, uint32_t version, uint32_t transition_version);
