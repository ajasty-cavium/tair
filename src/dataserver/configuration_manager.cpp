...
int configuration::get_conf_from_configfile(std::map<std::string, std::string> &conf, const std::vector<std::string> &lines)
{
...
int64_t configuration::process(request_admin_config *request, std::map<std::string, std::string> &configs)
{
  ...
  {
    TBSYS_LOG(INFO, "Process Config Request %s request opt %s",
    ...
    {
      TBSYS_LOG(INFO, "Config Key:%s Val:%s", iter->first.c_str(), iter->second.c_str());
    }
  }
  ...
  {
    TBSYS_LOG(INFO, "unknow %s request opt %d", request->cid().c_str(),
                                                request->opt());
    curt_version = -1;
  }
  unlock();
...
void configuration_manager::write_conf()
{
  ...
  do
  {
    ...
    if (fd == NULL)
    {
      TBSYS_LOG(ERROR, "open admin conf file %s failed: %s", admin_file_.c_str(), strerror(errno));
      break;
    }
    ...
    for (; it != configs_.end(); ++it)
    {
      ...
      for (; iter != kv.end(); ++iter)
      {
        ...
        key_value.reserve(iter->first.length() + iter->second.length() + 2); //~ plus key_delimiter, '\n' and '\t'
    ...
    {
      TBSYS_LOG(ERROR, "write admin conf error, file:%s, real write:%d", admin_file_.c_str(), len);
    }
    delete []output;
    fclose(fd);
  } while (0);
  ...
  {
    TBSYS_LOG(ERROR, "rename from %s to %s failed", tmp, admin_file_.c_str());
  }
}
...
int configuration_manager::do_load_admin_conf() const
{
  ...
  {
    TBSYS_LOG(ERROR, "snappy decompress err while GetUncompressedLength");
    return TAIR_RETURN_SUCCESS;
  }
  char *output = new char[output_length];
  if(!snappy::RawUncompress(buf, len, output))
  {
    TBSYS_LOG(ERROR, "snappy decompress err while RawUncompress");
    delete []buf;
    delete []output;
    return TAIR_RETURN_SUCCESS;
  }
...
void configuration_manager::register_configuration(configuration *conf)
{
  ...
  TBSYS_LOG(INFO, "Register configuration:%s", conf->cid().c_str());
}
