 ...
 *
 * Version: $Id: table_manager.hpp 2985 2014-10-10 09:46:32Z yunhen $
 *
 * Authors:
...
namespace tair {
   ...
   typedef __gnu_cxx::hash_map<int, migrate_unit, __gnu_cxx::hash<int> >::iterator bucket_server_map_it;
   ...
   public:
      ...
      /////////////////////////////
      void do_update_table(uint64_t *new_server_table, size_t size, uint32_t table_version, uint32_t copy_count, uint32_t bucket_count, bool calc_migrate = true);
      void direct_update_table(uint64_t *server_table, uint32_t server_list_count, uint32_t copy_count, uint32_t bucket_count, bool is_failover, bool version_changed, uint32_t server_table_version);
