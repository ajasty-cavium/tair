 ...
 *
 * Version: $Id: request_processor.cpp 2682 2014-06-26 08:30:17Z mingmin.xmm@alibaba-inc.com $
 *
 * Authors:
...
namespace tair {
  ...
  int request_processor::process(request_prefix_hides *request, bool &send_return, uint32_t &resp_size, uint32_t &weight)
  {
    easy_request_t *r = request->r;
    int rc = TAIR_RETURN_SUCCESS;
    if (tair_mgr->is_working() == false) {
      ...
      if (tair_mgr->should_proxy(request->key_start, target_server_id)) {
        ...
        if (plugin_ret < 0) {
          ...
          if (size == 0 || TAIR_RETURN_SUCCESS != rc) {
            ...
            delete result;
            result = NULL;
            log_info("no data return from get_range or err happen, rc:%d, key: %s", rc, request->key_start.get_data() + 4);
          } else {
  ...
  int request_processor::process(request_prefix_removes *request, bool &send_return, uint32_t &resp_size, uint32_t &weight)
  {
    easy_request_t *r = request->r;
    send_return = false;
    int rc = TAIR_RETURN_SUCCESS;
  ...
  int request_processor::process(request_prefix_incdec *request, bool &send_return, uint32_t &resp_size, uint32_t &weight)
  {
    easy_request_t *r = request->r;
    send_return = false;
    int rc = TAIR_RETURN_SUCCESS;
    ...
    if (tair_mgr->is_working() == false) {
      ...
      if (tair_mgr->should_proxy(*request->key_counter_map->begin()->first, target_server_id)) {
        ...
        plocker.lock(key);
        rc = tair_mgr->prefix_incdec(request, heart_beat->get_client_version(), low_bound, upper_bound, (req != NULL), resp);
        plocker.unlock(key);
      }
    }
  ...
  int request_processor::process(request_prefix_get_hiddens *request, bool &send_return, uint32_t &resp_size, uint32_t &weight)
  {
    easy_request_t *r = request->r;
    int rc = TAIR_RETURN_FAILED;
    response_prefix_gets *resp = new response_prefix_gets();
  ...
  bool request_processor::do_proxy(uint64_t target_server_id, base_packet *proxy_packet, easy_request_t *r) {
    ...
    if (server->send_proxy(addr, proxy_packet, r) == EASY_ERROR)       {
      TBSYS_LOG(ERROR, "easy_async_send error with null packet or handler");
      delete proxy_packet;
      return false;
    }
    return true;
