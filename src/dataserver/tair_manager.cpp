 ...
 *
 * Version: $Id: tair_manager.cpp 2985 2014-10-10 09:46:32Z yunhen $
 *
 * Authors:
...
namespace tair {
  tair_manager::tair_manager(easy_io_t *eio)
        : migrate_done_set(0), recovery_done_set(0)
        , current_bucket_count(1), total_bucket_count(1)
  ...
  int tair_manager::put(int area, data_entry &key, data_entry &value, int expire_time,base_packet *request,int heart_vesion, bool with_stat)
  {
                              ...
                              int *result_value, int expire_time,base_packet * request,int heart_version)
  {
    ...
    rc=storage_mgr->add_count(bucket_number, mkey, count, init_value, low_bound, upper_bound, expire_time, *result_value);
  ...
  int tair_manager::uniq_remove(int area, int bucket, data_entry &key)
  {
    ...
    log_debug("uniq_remove rc is %d, bucket num is %d, area is %d, key size is %d", rc, bucket_number, area, key.get_size());
  ...
  int tair_manager::batch_remove(int area, const tair_dataentry_set * key_list,request_remove *request,int heart_version)
  {
    ...
    for (it = key_list->begin(); it != key_list->end(); ++it)
    {
      ...
      if (rc == TAIR_RETURN_SUCCESS || rc == TAIR_RETURN_DATA_NOT_EXIST)
      {
        ...
        if (op_flag & TAIR_OPERATION_DUPLICATE) {
          log_error("mdelete shouldn't called in multi-copy cluster: area=%d, bucket_number=%d", area, bucket_number);
        }
      }
      else
      {
    ...
    switch (cmd) {
      ...
      case TAIR_ADMIN_SERVER_CMD_MODIFY_STAT_HIGN_OPS:
      {
        ...
        if (high_ops != 0 && high_ops != -1) {
          log_warn("modify stat_helper::stat_high_ops_count, %d -> %d", stat_helper::stat_high_ops_count, high_ops);
  ...
  void tair_manager::update_server_table(uint64_t *server_table, int server_table_size, uint32_t server_table_version,int32_t data_need_remove,
                                         vector<uint64_t> &current_state_table, uint32_t copy_count, uint32_t bucket_count, bool calc_migrate)
  {
    if (localmode == true) {
    ...
    table_mgr->do_update_table(server_table, server_table_size, server_table_version, copy_count, bucket_count, calc_migrate);
  ...
  void tair_manager::direct_update_table(uint64_t *server_table, uint32_t server_list_count, uint32_t copy_count, uint32_t bucket_count, bool is_failover,
                                         bool version_changed, uint32_t server_table_version)
  {
    ...
    table_mgr->direct_update_table(server_table, server_list_count, copy_count, bucket_count, is_failover, version_changed, server_table_version);
  }
  ...
  bool tair_manager::get_slaves(int server_flag, int bucket_number, vector<uint64_t> &slaves) {
    ...
    if (server_flag == TAIR_SERVERFLAG_PROXY || server_flag == TAIR_SERVERFLAG_RSYNC_PROXY) {
      ...
      table_mgr->get_slaves(bucket_number, (migrate_done_set.test(bucket_number) || recovery_done_set.test(bucket_number)), is_failover).swap(slaves);;
    }
    return is_failover;
  }
  ...
  int tair_manager::do_mc_ops(request_mc_ops *req, response_mc_ops *&resp, int heart_version)
  {
    ...
    switch (req->mc_opcode) {
      ...
      case 0x06: // DECR
      {
        ...
        rc = storage_mgr->incr_decr(bucket, mkey, delta, init, false/* is_incr */, req->expire, result, p_new_value);
        counter_result = bswap_64(result);
        val_buf = reinterpret_cast<char*> (&counter_result);
        val_len = sizeof(counter_result);
