 ...
 *
 * Version: $Id: tair_server.cpp 2985 2014-10-10 09:46:32Z yunhen $
 *
 * Authors:
...
namespace tair {
  ...
  bool tair_server::initialize()
  {
    ...
    //easy_eio_set_uthread_start(&eio, easy_helper::easy_set_thread_name<tair_server>, const_cast<char*>("ds_io"));
    if (task_thread_count > 0) {
      task_queue = easy_request_thread_create(&eio, task_thread_count, packet_handler_cb, this);
    }
    //~ no asynchronous tasks for the present
    //async_task_queue = easy_request_thread_create(&eio, 1, packet_handler_cb, this);
    ...
    slow_task_queue = easy_request_thread_create(&eio,
                                                 slow_task_thread_count,
                                                 packet_handler_cb,
                                                 this);
  ...
  bool tair_server::rsync_initialize()
  {
    ...
    if (rsync_enabled) {
      ...
      easy_eio_set_uthread_start(&rsync_eio, easy_helper::easy_set_thread_name<void>, const_cast<char*>("rsync_io"));
...
//      {
//        request_put *packet = static_cast<request_put*>(bp);
//        hv = easy_hash_code(packet->key.get_data(), packet->key.get_size(), 5);
//        break;
//      }
//      case TAIR_REQ_REMOVE_PACKET:
//      {
//        request_remove *packet = static_cast<request_remove*>(bp);
//        hv = easy_hash_code(packet->key->get_data(), packet->key->get_size(), 5);
//        break;
//      }
      default:
  ...
  int tair_server::packet_handler(easy_request_t *r)
  {
    ...
    if (!bp->control_cmd
          && easy_atomic_add_return(&cur_queued_mem, -(int64_t)bp->size()) >= max_queued_mem)
    {
      return EASY_OK;
    }
    ...
    if (bp->server_flag == TAIR_SERVERFLAG_CLIENT) {
      ...
      if ((bp->get_type() == base_packet::REQ_WRITE ||
        ...
        if (pcode == TAIR_REQ_PREFIX_PUTS_PACKET ||
          ...
          response_return *return_packet = new response_return(bp->getChannelId(),
              TAIR_RETURN_READ_ONLY, "your namespace has been set readonly",
              heartbeat.get_client_version());
    ...
    if (EASY_OK == easy_ret) {
      ...
      if (r->ipacket != NULL && ((base_packet*)r->ipacket)->is_hot()) {
        ...
        feedback->_next = resp;
        r->opacket = feedback;
        log_warn("send response_feedback to %s, ns: %u",
            tbsys::CNetUtil::addrToString(req->peer_id).c_str(), req->ns());
      }
    }
  ...
  void tair_server::load_max_queued_mem()
  {
    ...
    max_queued_mem = strtoll(max_queued_mem_str, NULL, 10);
    log_warn("max_queued_mem is %"PRI64_PREFIX"u", max_queued_mem);
  }
