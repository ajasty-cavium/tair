...
namespace tair{
dup_sync_sender_manager::dup_sync_sender_manager(tair::stat::FlowController *flow_ctrl, RTCollector *rtc, uint32_t ioth_count)
{
  ...
  eio.tcp_cork = 0;
  easy_io_start(&eio);
  easy_eio_set_uthread_start(&eio, easy_helper::easy_set_thread_name<dup_sync_sender_manager>, const_cast<char*>("dup_io"));
    ...
    base_packet *request)
{
  ...
  log_debug("duplicate packet %d sent: %s",tmp_packet->packet_id,tbsys::CNetUtil::addrToString(des_server_ids[0]).c_str());
  if (easy_helper::easy_async_send(&eio, des_server_ids[0], tmp_packet, request->r, &handler, dup_timeout) != EASY_OK) {
    log_error("duplicate packet failed, packet_id: %d, slave: %s",
        tmp_packet->packet_id, tbsys::CNetUtil::addrToString(des_server_ids[0]).c_str());
    delete tmp_packet;
    return TAIR_RETURN_DUPLICATE_BUSY;
  ...
  switch (pcode) {
    ...
    case TAIR_REQ_MC_OPS_PACKET:
    {
      ...
      if (resp->mc_opcode == 0x05 /* INCR */
        ...
        resp->value.set_alloced_data(req->padding, sizeof(uint64_t));// first 8 bytes is value, last 2 bytes is version
        req->padding = NULL;
      }
...
int dup_sync_sender_manager::packet_handler(easy_request_t *r) {
  ...
  if (rc == EASY_ERROR) {
    ...
    log_error("duplicate to %s timeout, pcode : %d, ns : %d", easy_helper::easy_connection_to_str(r->ms->c).c_str(), pcode, ns);
  } else {
    if (cr != NULL) {
