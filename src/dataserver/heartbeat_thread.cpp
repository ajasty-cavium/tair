 ...
 *
 * Version: $Id: heartbeat_thread.cpp 2922 2014-09-05 08:25:26Z yunhen $
 *
 * Authors:
  ...
  void heartbeat_thread::run(tbsys::CThread *thread, void *arg)
  {
    ...
    while (!_stop) {
      ...
      if (health_check)
      {
        ...
        base_packet *bp = (base_packet*)easy_helper::easy_sync_send(&eio, local_server_ip::ip, (void *)(new request_ping()), NULL, &handler);
            ...
            if (need_table_updater) {
              ...
              server_table_updater *stu =
                new server_table_updater(tair_mgr, server_list, resp->server_list_count, server_version,
                                         resp->transition_info.data_need_move ? 1 : 0, resp->migrated_info,
              ...
              uint64_t *server_list = resp->get_server_list(resp->bucket_count, resp->copy_count);
              if (NULL != server_list) {
                log_debug("direct update %d %d %d", resp->server_list_count, resp->copy_count, resp->bucket_count);
                tair_mgr->direct_update_table(server_list, resp->server_list_count, resp->copy_count, resp->bucket_count,
              ...
              if (resp->transition_info.failovering) {
                recovery_bucket_updater *rbu = new recovery_bucket_updater(tair_mgr, resp->recovery_ds, server_version, transition_version);
                rbu->start();
                rbu->stop();
              }
              ...
              if (NULL != server_list) {
                log_debug("direct update %d %d %d", resp->server_list_count, resp->copy_count, resp->bucket_count);
                tair_mgr->direct_update_table(server_list, resp->server_list_count, resp->copy_count, resp->bucket_count, resp->transition_info.failovering);
              } else {
                log_error("serverTable is NULL");
              }
          ...
          if (need_set && !resp->vec_area_capacity_info.empty() && !tair_mgr->is_localmode()) {
            ...
            for (vector<pair<uint32_t, uint64_t> >::iterator it = resp->vec_area_capacity_info.begin();
              ...
              if (it->first >= 0 && it->first < TAIR_MAX_AREA_COUNT) {
                ...
                log_info("area %u, capacity %" PRI64_PREFIX "u", it->first, it->second);
