...
static int deliver_websocket_frame(struct connection *conn) {
  ...
  if (buffered) {
    ...
    if (conn->endpoint.uh->handler((&conn->mg_conn)->server_param, &conn->mg_conn)) {
...
static void call_uri_handler_if_data_is_buffered(struct connection *conn) {
  ...
  if (conn->mg_conn.is_websocket) {
    ...
    conn->endpoint.uh->handler(c->server_param, c);
    if (conn->flags & CONN_HEADERS_SENT) {
...
// free server config_options
static void free_option_values(char **opts)
{
...
void mg_destroy_server(struct mg_server **server) {
  ...
  if (server != NULL && *server != NULL) {
    ...
    free_option_values((*server)->config_options);
    LINKED_LIST_FOREACH(&(*server)->active_connections, lp, tmp) {
