 ...
 *
 * Version: $Id: stat_define.hpp 1484 2013-04-22 05:09:19Z nayan $
 *
 * Authors:
...
namespace tair
{
	namespace common
	{
		template<typename T> void any_cleanup(void* a)
		{
...
#pragma pack()
		...
		public:
			...
			void update(const StatUnit<Store>& diff, const std::vector<int32_t>* types, int32_t offset, const StatSchema* schema);
		...
		template<typename StatUnit, typename StatStore> class StatManager
		{
		public:
			...
			void reset_key(int32_t key, int32_t score = 0, const std::vector<int32_t>* types = NULL, const StatSchema* schema = NULL);
			// clear key => stat
