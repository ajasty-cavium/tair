    ...
    class BucketQueue
    {
      ...
      public:
        class Options {
          ...
          private:
            inline void set_record_size_limit(int l)
            { record_size_limit_ = (l <= 0 ? SIZE_LIMIT : l); }

            inline void set_record_count_limit(int l)
            { record_count_limit_ = (l <= 0 ? COUNT_LIMIT : l); }

            inline void set_fail_wait_time(int t)
            { fail_wait_time_ = (t < 0 ? WAIT_TIME : t); }

            inline void set_queue_max_size(int s)
            { queue_max_size_ = (s <= 0 ? QUEUE_MAX_SIZE : s); }

            inline void set_log_buffer_size(int sz)
            { log_buffer_size_ = (sz <= 0 ? LOG_BUFFER_SIZE : sz); }
          private:
