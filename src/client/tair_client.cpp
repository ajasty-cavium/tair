 ...
 *
 *
 * Version: $Id: tair_client.cpp 2935 2014-09-09 09:35:24Z yunhen $
 *
 * Authors:
   ...
   void tair_client::print_help(const char *cmd)
   {
      ...
      if (cmd == NULL || strcmp(cmd, "active_failover") == 0) {
         fprintf(stderr,
                 ...
                 "\tds_addr: dataserver to do active failover, eg: 192.168.0.1:5191\n"
            );
      }
      ...
      if (cmd == NULL || strcmp(cmd, "active_recover") == 0) {
         fprintf(stderr,
                 ...
                 "\tds_addr: dataserver to do active recover, eg: 192.168.0.1:5191\n"
            );
      }
      ...
      if (cmd == NULL || strcmp(cmd, "replace_ds") == 0) {
         fprintf(stderr,
                 ...
                 "\tsrc_ds: dataservers to be replaced, eg: 192.168.0.1:5191,192.168.0.2:5191\n"
            );
      }
      ...
      if (cmd == NULL || strcmp(cmd, "urgent_offline") == 0) {
         fprintf(stderr,
                 ...
                 "\tsrc_ds: dataserver which is already down, eg: 192.168.0.1:5191\n"
                 "\tdst_ds: dataservers which to replace down server, eg: 192.168.0.1:5191,192.168.0.2:5191\n"
            );
      }
      ...
      if (cmd == NULL || strcmp(cmd, "ldb_instance_config") == 0) {
         fprintf(stderr,
                 ...
                 "EXAMPLE: ldb_instance_config add_gc_bucket 100,200 0,2,6 192.168.1.100:5191\n"
            );
      }
...
// remove
  ...
  void tair_client::set_flow_limit_bound(int ns, int lower, int upper, tair::stat::FlowType type)
  {
   ...
   void tair_client::do_cmd_op_ds(const vector<string>& param, const char* cmd_str, ServerCmdType cmd_type, int base_param_size, op_cmd_result_map& result)
   {
     if (static_cast<int>(param.size()) < base_param_size || static_cast<int>(param.size()) > base_param_size + 1)
     {
       print_help(cmd_str);
