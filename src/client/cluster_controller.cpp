 ...
 *
 *
 * Version: $Id: tair_mc_client_api.hpp 1245 2014-03-03 13:58:28Z huan.wanghuan@alibaba-inc.com $
 *
 * Authors:
...
namespace tair
{
  cluster_controller::cluster_controller(): timeout_ms(TAIR_DEFAULT_TIMEOUT), client_vector_rwlock(tbsys::WRITE_PRIORITY)
  {
  ...
  bool cluster_controller::extract_cluster_config(cJSON* root, cluster_info_vector& cluster_config)
  {
    ...
    for (int32_t i = 0; i < clusters_num; ++i)
    {
      ...
      {
        TBSYS_LOG(ERROR, "diamond information miss tair configserver master address");
        return false;
      }
      ...
      if (NULL != slave && NULL != slave->valuestring)
        ...
        TBSYS_LOG(WARN, "diamond information has no tair configserver slave address");

      // group
      cJSON* group = cJSON_GetObjectItem(address, "group");
      if (NULL == group || NULL == group->valuestring)
      {
        TBSYS_LOG(ERROR, "diamond information miss tair configserver group address");
        return false;
      }
      ...
      if (NULL != namespace_offset_json && NULL != namespace_offset_json->valuestring)
        ...
        TBSYS_LOG(WARN, "diamond information has no namespace_offset");
      if ((one_cluster_config->namespace_offset < 0) || (one_cluster_config->namespace_offset >= TAIR_MAX_AREA_COUNT))
      {
        TBSYS_LOG(ERROR, "diamond config info namespaceOffset %d out of range", one_cluster_config->namespace_offset);
        return false;
      }
  ...
  bool cluster_controller::update_config(const string& json_str)
  {
    ...
    if (NULL == root)
    {
      TBSYS_LOG(ERROR, "parse json info failed, at:\n %s\n", cJSON_GetErrorPtr());
      return false;
    }
  ...
  bool cluster_controller::update_config_cluster(const cluster_info_vector& cluster_config)
  {
    ...
    {
      TBSYS_LOG(ERROR, "There is no tair cluster to read");
      return false;
    }
    if (temp_write_cluster->empty())
    {
      TBSYS_LOG(ERROR, "There is no tair cluster to write");
      return false;
    }
    ...
    TBSYS_LOG(WARN, "%s", log_info.str().c_str());
    return true;
  }
